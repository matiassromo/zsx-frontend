              ...
              entranceTransactions.map((entranceTransaction) => (
                <tr key={entranceTransaction.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm text-gray-900">
                    <div className="font-medium">{entranceTransaction.owner?.name || 'Sin cliente'}</div>
                    <div className="mt-1 text-xs text-gray-500 md:hidden">
                      {formatDateTime(entranceTransaction.entryTime)}{' '}
                      {entranceTransaction.exitTime ? `- ${formatDateTime(entranceTransaction.exitTime)}` : '- En curso'}
                    </div>
                    <div className="text-xs text-gray-500 md:hidden">Personas: {formatCountsShort(entranceTransaction)}</div>
                  </td>

                  <td className="px-4 py-3 text-sm text-gray-700">
                    <div>
                      <p className="font-medium text-gray-900">{formatTransactionId(entranceTransaction.transactionId)}</p>
                      {entranceTransaction.transactionId && (
                        <p className="text-xs text-gray-400 font-mono">{entranceTransaction.transactionId}</p>
                      )}
                    </div>
                  </td>

                  <td className="hidden md:table-cell px-4 py-3 text-sm text-gray-500">{formatDateTime(entranceTransaction.entryTime)}</td>
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700">{formatCurrency(entranceTransaction.total)}</td>

                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <IconButton label="Editar" onClick={() => onEdit(entranceTransaction)}>
                        <PencilIcon className="h-5 w-5" />
                      </IconButton>

                      <IconButton label="Eliminar" variant="danger" onClick={() => onDelete(entranceTransaction)}>
